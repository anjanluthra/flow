...
export async function GET(request: NextRequest) {
  try {
    ...
    const actuals = new Map<number, { income: number; expense: number }>()
    for (let m = 1; m <= 12; m++) actuals.set(m, { income: 0, expense: 0 })
    ...
    const forecasts = new Map<number, { income: number; expense: number }>()
    ...
    const months = Array.from({ length: 12 }, (_, i) => {
      ...
      const hasActuals = a.income > 0 || a.expense > 0
      return {
        month: m,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { year, month, forecastIncome, forecastExpense, notes } = body as {
      year: number
      month: number
